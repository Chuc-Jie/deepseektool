...
### 新增
- **对话文件夹管理**（并入自独立脚本 waitadd/deepseek-folder-manager，对齐 v0.9.2 树形交互）：左侧对话历史栏加入「文件夹」面板。文件夹以树形就地展开/收起（空心文件夹图标、展开态持久化），其下内嵌该文件夹的会话行（当前会话高亮、可点击跳转官方会话、每行「移出」）；每行可通过 ⋯ 菜单 →「移动到文件夹」归入/移出（悬停即开的级联浮层 + 标题彩色标签）；**已收进子文件夹的会话从原生历史列表自动隐藏**，避免点树内会话时官方滚动回原位（路由切换才重绘树，observer 不会自激）。
