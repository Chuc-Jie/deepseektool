...
### 新增
- **对话文件夹管理**（并入自独立脚本 waitadd/deepseek-folder-manager v0.8.1）：左侧对话历史栏加入「文件夹」分类面板，通过会话 ⋯ 菜单 →「移动到文件夹」把对话归入/移出文件夹，复用三点菜单悬停即展开的级联浮层；会话抽取过滤 + 标题文件夹标签
- **控制面板新增开关卡片**「🗂 对话文件夹管理」：默认**关闭**（opt-in）。关闭时不注入侧栏面板/抽屉/菜单项/样式，且会整体清理已应用的 DOM（隐藏恢复、标签移除、styles/CSS 变量移除）
- **并入细节**
  - 数据存独立新键 `deepseek_folder_manager_data_v2`，与旧独立脚本（`ds_folder_manager_v1` 键）互不干扰、不动老数据
